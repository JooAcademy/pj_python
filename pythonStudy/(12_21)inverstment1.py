...
for row in rows[1:]:
    stock_name = row.select_one('td:nth-child(2) > a').text
    current_price = row.select_one('td:nth-child(3)').text
    print(f'종목명: {stock_name}, 현재가: {current_price}')
'''



stock_name = soup.select_one(f"#contentarea > div.box_type_l > table.type_2 > tbody > tr:nth-child(10) > td:nth-child(2) > a")
current_price = soup.select_one(f"#contentarea > div.box_type_l > table.type_2 > tbody > tr:nth-child(10) > td:nth-child(3)")
print(f"{stock_name.text} 현재가: {current_price.text}")


#contentarea > div.box_type_l > table.type_2 > tbody > tr:nth-child(10) > td:nth-child(2) > a

#contentarea > div.box_type_l > table.type_2 > tbody > tr:nth-child(14) > td:nth-child(2) > a
