    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == K_LEFT:
                ...
                PADDLE.rect.centerx += 10
    for BALL in BALLS:
        if BALL.rect.centery < 1000: # 아직 게임이 끝나지 않았으면
            BALL.move()

        # 블록과 충돌하면
        prevlen = len(BLOCKS) # 충돌전의 블록을 세어 prevlen에 저장
        BLOCKS = [x for x in BLOCKS
                if not x.rect.colliderect(BALL.rect)] # 공과 블럭이 충돌하지 않은것만 다시 블럭에 재설정
        ...
        if PADDLE.rect.colliderect(BALL.rect):
            BALL.dir = 90 + (PADDLE.rect.centerx - BALL.rect.centerx) \
                / PADDLE.rect.width * 80 # 공이 패들에 오른쪽에 맞으면 오른쪽으로 왼쪽에 맞으면 왼쪽으로 공의 방향이 바뀌는 코드

        # 벽과 충돌하면
        if BALL.rect.centerx < 0 or BALL.rect.centerx > 600:
            BALL.dir = 180 - BALL.dir
        if BALL.rect.centery < 0: # 공이 위벽과 충돌하면 
            BALL.dir = -BALL.dir # 공의 방향이 바뀌어진다. 
            BALL.speed = 15 

pygame.init()
pygame.key.set_repeat(5, 5) # 누르고 있는 키의 반복을 제어
SURFACE = pygame.display.set_mode((600, 800)) # 창설정 
FPSCLOCK = pygame.time.Clock() # CPU의 부하를 줄여주기 위한 코드 
BLOCKS = [] # 블록의 리스트를 초기화 함
PADDLE = Block((242, 242, 0), Rect(300, 700, 100, 30)) # PADDLE을 그리기 위한 코드
BALLS = [Block((242, 242, 0), Rect(300, 400, 20, 20), 10)] # 공을 그리기 위한 코드 

isNeedToRestart = False # 처음에 재시작은 안하는걸로 설정
isFeverTime = False # 피버타임은 처음엔 없는걸로 설정
score = 0 # 점수초기화
startTime = 0.0 # 시작시간초기화
endTime = 0.0 # 마침 시간 초기화
